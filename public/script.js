...
document.getElementById("fontSize").addEventListener("change", function () {
  ...
  document.execCommand("fontSize", false, "7"); // Use largest size as a placeholder

  // Find all <font size="7"> tags and set their style to the selected px value
  const editor = document.getElementById("editor");
  ...
  updateCounts(); // Add this line to update word and character count
});

// Line Spacing
document.getElementById("lineSpacingSelect").addEventListener("change", function () {
  const spacing = this.value;
  // Apply line spacing to the selected text or the whole editor
  ...
  if (selection && !selection.isCollapsed) {
    ...
    // Apply to the whole editor if nothing is selected
    editor.style.lineHeight = spacing;
  }
});

// Optional: clicking the icon focuses the dropdown
document.getElementById("lineSpacingBtn").addEventListener("click", function () {
  document.getElementById("lineSpacingSelect").focus();
});
